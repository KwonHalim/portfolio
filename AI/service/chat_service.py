...
class ChatService:
    ...
    def ask(self, question: str, session_id: str) -> Dict[str, str]:
        ...
        # 1. 검색기를 호출하여 컨텍스트와 참조 문서를 가져옵니다.
        ...
        # 2. 프롬프트, LLM, 출력 파서를 연결하여 RAG 체인을 구성합니다.
        chain = self.prompt | self.llm | StrOutputParser()

        # 3. 체인을 실행하여 AI의 답변을 생성합니다.
        answer = chain.invoke({
            "context": context,
            "question": question
        })

        # 4. 대화 내용을 DB에 저장하기 위한 메타데이터를 구성합니다.
        metadata = {
            "retrieved_source_ids": source_ids
        }

        # 5. 대화 내용을 저장하고, 생성된 chat_id를 받습니다.
